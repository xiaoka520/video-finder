...
resolve_dir():
  ...
  last_dir = tracking/downloads.json 最后一条 completed 的 output_dir
            ?? history.json 最后一条记录的 download_dir
...
yt-dlp \
  ...
  -f "bestvideo[height<=?{quality}][ext=mp4]+bestaudio[ext=m4a]/best[height<=?{quality}]"   ← quality_min 为"不限"时回退为 -f "best"
  -o "{dir}/%(title)s.%(ext)s"
  --newline
...
- `{quality}` 从 `preferences.quality_min` 提取数字（1080p → 1080，4K → 2160，"不限" → 省略 height 过滤，直接用 `best`）
  ...
  "completed": [
    {
      ...
      "title": "Passionate sex of real amateur couple",
      "output_dir": "~/Downloads/movies",
      "filename": "Passionate sex.mp4",
      "size_bytes": 488636416,
      "completed_at": "2026-07-06T14:35:00+08:00"
    }
  ],
  "failed": [
    {
      "id": "1718123456-c7d8e9",
      "title": "...",
      "output_dir": "~/Downloads/movies",
      "reason": "下载超时（> 2 小时）",
      "failed_at": "2026-07-06T16:30:00+08:00"
    }
  ]
}
```
...
| `output_dir` | 下载目录（active → completed 时保留，供下次回退查询） |

## 其他站点

不在 `preferred_sites` 中的站点：
1. 直接用 yt-dlp 通用格式 `yt-dlp -f best "{url}" --simulate` 试下载（`--simulate` 不实际下载，仅验证 URL 是否支持）
2. 支持 → 去掉 `--simulate` 正式下载
3. 报错 \"Unsupported URL\" / \"No video formats found\" → 告知用户该站点暂不支持，询问是否换源
4. 网络错误 → 触发代理重试（→ video_finder_proxy）
